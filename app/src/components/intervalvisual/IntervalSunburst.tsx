...
export const IntervalSunburst: React.FunctionComponent = () => {
  ...
  const {
    ...
    data
  } = useContext(IntervalVisualContext);
  ...
  const renderSvg = () => {
    ...
    const nRadius = (Math.min(width, height) * 0.80);
    setRadius(nRadius);
  ...
  useEffect(() => {
    ...
    if (! intervalNode) {
      consoleLog({prefix: 'IntervalSunburst', message: 'Error: Cannot traverseToViewNode due to failure to find navigated interval ' + interval._id});
      return;
    }
    ...
    if (intervalNode.children) {
      ...
      //
      consoleLog({prefix: 'IntervalSunburst', message: 'traverseToViewNode ' + intervalNode.parent.id + ' as is parent of ' + intervalNode.id});
  ...
  const handleNavigate = (intervalNode: ViewNode) => {
    ...
    if (intervalNode === parent) {
      consoleLog({prefix: 'Navigate', message: intervalNode.data.id() + " is already the parent so setting its parent as parent: " + ( intervalNode.parent?.data.id() || rootNode.data.id() )})
      ...
      >
        ...
        <g id="interval-container" className="interval-container">
          <IntervalSunburstSegments
            ...
            parent={parent}
            radius={radius} zoomSystem={zoomSystem}
            selected={selected} setSelected={handleSelection}
            navigate={handleNavigate} />

          <IntervalSunburstParent
            parent={parent} radius={radius} zoomSystem={zoomSystem}
