...
export const IntervalSunburstParent: React.FunctionComponent<SunburstParentProps> = (props: SunburstParentProps) => {
  ...
  const handleDoubleClick = (event: React.MouseEvent<SVGCircleElement, MouseEvent>, parentNode: ViewNode|undefined) => {
  ...
  return (
    <React.Fragment>
      <circle
        ...
        id="parent-circle" className={props.parent.data.selected ? 'path-selected' : 'path-unselected'}
        r={props.radius} fill="url(#parentGradient)" pointerEvents="all"
        style={{cursor: 'pointer'}} transform={centreTranslation}
      ...
      <text
        ...
        id="parent-label" pointerEvents="none" textAnchor="middle"
        style={{userSelect: 'none', fontWeight: 'bold'}} dy="0.35em"
