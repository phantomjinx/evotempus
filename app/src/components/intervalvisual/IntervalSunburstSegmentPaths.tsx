...
import { consoleLog, displayYear } from 'src/utils';
...
export const IntervalSunburstSegmentPaths: React.FunctionComponent<SunburstSegmentPathsProps> = (props: SunburstSegmentPathsProps) => {
  ...
  const segmentPath = (node: ViewNode) => {
    ...
    return (
      <animated.path
        id={'path-' + node.data.id()} className={classes} key={node.data.id()}
  ...
  return (
    <React.Fragment>
      {props.nodes.map(node => segmentPath(node))}
    </React.Fragment>
  )
}
